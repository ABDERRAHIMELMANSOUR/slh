...
export default function BlogPost() {
  const {slug}=useParams(); const {t,lang,path}=useLang(); const {blog,settings}=useData()
  const post=blog.find(p=>p.slug===slug)
  useSEO({lang,pageKey:'blogPost',params:{slug},title:post?`${post.title} | SLH Service Nederland`:`${t.blog.notFound} | SLH Service Nederland`,description:post?.description||t.blog.subtitle,noindex:!post})
  if (!post) return <Layout><div className="pt-32 pb-24 text-center wrap"><h1 className="text-3xl font-extrabold text-slate-800 mb-4" style={{ fontFamily:"'Sora',sans-serif" }}>{t.blog.notFound}</h1><Link to={path('blog')} className="btn-primary gap-2"><ArrowLeft size={16}/>{t.blog.viewAll}</Link></div></Layout>
  return (
    <Layout>
      <div className="relative pt-32 pb-20 overflow-hidden" style={{ background:'linear-gradient(160deg,#051C38,#0D3A6E,#005E78)' }}>
        ...
        <div className="wrap relative z-10 max-w-4xl">
          <Link to={path('blog')} className="inline-flex items-center gap-2 text-white/55 hover:text-white text-sm font-medium mb-8 transition-colors"><ArrowLeft size={14}/>{t.blog.viewAll}</Link>
      ...
      <article className="py-16 bg-white">
        <div className="wrap max-w-4xl">
          ...
          <div className="text-slate-600 leading-relaxed">
            ...
            <p className="mb-4">{t.blog.articleIntro}</p>
            <p>{t.blog.articleContact} <a href={`mailto:${settings.email}`} className="text-cyan hover:underline">{settings.email}</a>.</p>
          </div>
